...
不用记命令，也不用先写模板。告诉 Agent 你想做什么、遇到什么问题，或者想找什么工具。
...
Skill 会根据任务选择搜索、代码、文档等合适的工具，按步骤推进；方向不清楚时先问你一个关键问题。
...
它会查看实际文档和代码，检查功能是否真的存在。只在 README 里写了、但没有确认的内容，会明确告诉你。
...
你会得到整理好的结果、依据和限制，知道哪个方案能用、哪里要改，以及下一步怎么做。
